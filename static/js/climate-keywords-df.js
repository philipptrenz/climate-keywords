require(['c3', 'jquery'], function(c3, $) {
    ...
    function draw_stats(jsonData) {
        ...
        Object.keys(jsonData['corpora']).forEach(function(corpus_name) {
            var corpus_div_class = "corpus-stats-" + corpus_name.replace(/_/g, '-').replace(/ /g, '-');

            $( "#row-stats-container" ).append(`
                <div class="col-md-${Math.round(12.0 / Object.keys(jsonData['corpora']).length)}">
                    <div class="card">
                        <div class="card-header"><h3 class="card-title">${corpus_name.replace(/_/g, ' ').replace(/ /g, ' ')}</h3></div>
                        <div class="card-body" id="${corpus_div_class}" style="height: 15rem; padding: 0;"></div>
                    </div>
                </div>
            `)
            ...
            keywordStatsCharts[corpus_name] = c3.generate({
                bindto: '#' + corpus_div_class, // id of chart wrapper
                data: {
                    x: 'x',
                    columns: [
                        [ 'x' ].concat(jsonData['labels']),
